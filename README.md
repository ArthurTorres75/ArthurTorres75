<!--
**ArthurTorres75/ArthurTorres75** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...

- 🔭 I’m currently working on web development MERN, and Full stack development AWS + Terraform, Nextjs and INFRA Typescript  
- 🌱 I’m currently learning AWS and terraform and Three.js
- 💬 Ask me about Angular, Typescript, React, TailwindCSS, Next JS, Nest JS, Node JS. CSS and animations.
- 📫 How to reach me: arthurtorres75@gmail.com, https://www.upwork.com/freelancers/~0110023d7209510ffb, https://arthurtorres75.github.io/portfolio/
-->

# Hi, I'm Arthur 👋
![](https://komarev.com/ghpvc/?username=ArthurTorres75&color=green) <br />
### Full Stack Engineer | Scalable Web Applications | Clean Architecture

I build modern, high-performance web applications focused on scalability, maintainability, and excellent user experience.

---

## 🚀 About Me

- 💻 Full Stack Developer specialized in frontend & backend systems
- ⚡ Strong experience with modern JavaScript / TypeScript ecosystems
- 🏗️ Passionate about clean architecture and scalable solutions
- 🌍 Available for remote opportunities worldwide
- 📍 LATAM Timezone

---

## 🛠️ Tech Stack

### Frontend
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react)
![Next JS](https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=next.js)
![Angular](https://img.shields.io/badge/Angular-DD0031?style=for-the-badge&logo=angular)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript)
![Tailwind](https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=for-the-badge&logo=tailwind-css)
![Shadcn UI](https://img.shields.io/badge/shadcn/ui-000000?style=for-the-badge)

### UI / Motion / 3D
![Three.js](https://img.shields.io/badge/Three.js-000000?style=for-the-badge&logo=three.js)
![Framer Motion](https://img.shields.io/badge/Framer_Motion-black?style=for-the-badge&logo=framer)
![GSAP](https://img.shields.io/badge/GSAP-88CE02?style=for-the-badge)
![Anime.js](https://img.shields.io/badge/Anime.js-111111?style=for-the-badge)

### Backend
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=node.js)
![NestJS](https://img.shields.io/badge/NestJS-E0234E?style=for-the-badge&logo=nestjs)
![PHP](https://img.shields.io/badge/PHP-777BB4?style=for-the-badge&logo=php)

### Database & Cloud
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=for-the-badge&logo=postgresql)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonaws)
![Cloudflare](https://img.shields.io/badge/Cloudflare-F38020?style=for-the-badge&logo=cloudflare)

---

## 💼 Featured Projects

### 🔹 SaaS Dashboard
Admin platform with analytics, authentication, subscriptions, and role permissions.

### 🔹 E-commerce Platform
Modern store with payments, cart, product management, and responsive design.

### 🔹 POS System
Sales & inventory system for small businesses optimized for mobile devices.

### 🔹 REST API Architecture
Scalable backend using NestJS, JWT auth, modules, and clean code practices.

---

## 📈 GitHub Stats

![Arthur GitHub stats](https://github-readme-stats.vercel.app/api?username=TUUSERNAME&show_icons=true&theme=tokyonight)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=TUUSERNAME&layout=compact&theme=tokyonight)

---

## 🎯 What I Focus On

- Scalable Architecture
- Clean Code
- Performance Optimization
- UI / UX Quality
- Business-Oriented Solutions

---

## 📩 Contact

- 💼 LinkedIn: YOUR_LINKEDIN
- 🌐 Portfolio: YOUR_WEBSITE
- 📧 Email: YOUR_EMAIL

---

## ⚡ Motto

> Great software is not only built to work today, but to grow tomorrow.
